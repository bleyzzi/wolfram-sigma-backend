...
user_account_page = APIRouter(tags=["user-account"], dependencies=[Depends(current_user)])


@user_account_page.get("/get_user_account_info")
async def get_user_account_info(user: User = Depends(current_user)) -> UserSchema:
    user_get = await UserService.get_user(user.id)
    return user_get


@user_account_page.put("/edit_user_account")
async def edit_user_account(user_schema: UserEditSchema, user: User = Depends(current_user)) -> UserSchema:
